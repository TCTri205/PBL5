# 🍎 API Reference: `fruit_classifier.py`

Tài liệu tham khảo API cho script phân loại trái cây sử dụng ONNX Runtime trên Raspberry Pi.

## Tổng Quan

`fruit_classifier.py` là script độc lập dùng để chạy inference (dự đoán) trên mô hình YOLO đã được export sang định dạng ONNX. Script được thiết kế tối ưu cho Raspberry Pi 4, sử dụng CPUExecutionProvider để đảm bảo ổn định.
...
```bash
python fruit_classifier.py <model_path.onnx> <image_path>
```
...
2. **Model file không tồn tại**
   ...
   ```bash
   python fruit_classifier.py wrong_path.onnx image.jpg
...
*Generated from `repo/pi_edge/fruit_classifier.py`*
