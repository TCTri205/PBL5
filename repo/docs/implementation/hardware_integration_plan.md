...
- **Đặc tả Code**:
  ...
  - **MỚI**: Định nghĩa hàm async: `async def wait_until_clear(self, timeout=5.0)`: Chờ cho đến khi vật thể đi qua hết (sensor không còn bị che) để tránh chụp lặp cùng một quả. **Timeout mặc định là 30.0 giây** (nhất quán với cách gọi trong `run_pipeline`).
...
- **Hành động**:
  ...
  - **4.3**: Thay đổi logic trong `run_pipeline()`:
    ...
    - **MỚI**: Gọi `await self.conveyor.wait_until_clear()` để chắc chắn vùng cảm biến đã trống.
  ...
  - **4.7**: Bổ sung flag `--capture-delay` và `--resume-delay` vào argument parser.
